# mockgrid_dust_2.py configuration file
# Ben Cook (bcook@cfa.harvard.edu)

###############################################
# CONFIG FILE for mock test
# MOCK Galaxy: has tau=2 SFH, lognormal Dust model
...
lower_sfh = np.log10(agemodels.TauModel(np.array([0.5, 2.]), iso_step=-1).SFH)
upper_sfh = np.log10(agemodels.TauModel(np.array([3.0, 2.]), iso_step=-1).SFH)
...
metals = metalmodels.SingleFeH  # Single FeH
...
# Tau model with [Fe/H]=-0.2, log E(B-V) = -.5, sig(E(B-V)) = 0.5
# Npix = 1e2, tau=2 Gyr
gal_params = np.array([-0.2, -0.5, 0.5, 2., 2.])
