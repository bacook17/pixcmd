...
Npix_low = 3.0
model = ppy.sfhmodels.ConstantSFR(iso_step=-1)
model.set_params([Npix_low])
lower_sfh = np.log10(model.SFH)

Npix_high = 5.0
model.set_params([Npix_high])
...
dustmodel = dustmodel
sfhmodel = ppy.sfhmodels.ConstantSFR()
distancemodel = ppy.distancemodels.VariableDistance()  # dmod floats
...
gal_params = np.array([feh, log_ebv, log_npix, dmod])
