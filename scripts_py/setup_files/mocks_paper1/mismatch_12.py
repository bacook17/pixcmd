...
###############################################
# CONFIG FILE for Mismatch Run #12
# MOCK Galaxy:
#    FeH Model: Increases linearly with log time
#
...
metalmodel = ppy.metalmodels.SingleFeH()
dustmodel = dustmodel
sfhmodel = sfhmodel
distancemodel = ppy.distancemodels.VariableDistance()  # dmod floats
...
# Temporarily override the metallicity model to return different metallicity at each age
def temp_iter_SSPs():
    temp_feh = np.linspace(0.25, -1.25, len(model_mock.ages))
    for i in range(model_mock.num_SSPs):
        yield model_mock.ages[i], temp_feh[i], model_mock.SFH[i], model_mock.dmod


model_mock.iter_SSPs = temp_iter_SSPs
