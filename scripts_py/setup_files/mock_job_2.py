...
# dust = -0.7 +/- 0.1
# dist_mod = 29.4
# SFH 0-3 : (Npix, tau) = (3.2, 8.0)
# SFH 4-5 : (Npix, tau) = (3.0, 2.0)
#
# INITIAL MODEL Galaxy: Tau model, fixed MDF, fixed dust width, var. distance
...
_nlive_final = 1000
if DYNAMIC:
    init_run_params['nlive_init'] = _nlive
    final_run_params['nlive_init'] = _nlive_final
else:
    init_sampler_params['nlive'] = _nlive
    final_sampler_params['nlive'] = _nlive_final
...
params['shot_noise'] = False

###############################################
# PRIOR SETTINGS

# The bounds on the flat prior for each parameter
z_bound = [-1.5, 0.25]  # metallicity
...
agemodel = ppy.agemodels.NonParam()  # NonParam SFH
distancemodel = ppy.distancemodels.FixedDistance(29.4)
...
gal_params += list(ppy.agemodels.TauModel([3.2, 8.0]).as_default().logSFH[:4])
gal_params += list(ppy.agemodels.TauModel([3.0, 2.0]).as_default().logSFH[4:])
