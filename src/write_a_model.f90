PROGRAM WRITE_A_MODEL
  ...
  !syntax: write_a_model.exe Mpix SFH_type tau/log_age tag
  ...
  IF (sfhflag.EQ.1) THEN
     outfile = 'model_M'//TRIM(char_mpix(1:3))//'_SFH'//&
          TRIM(char_flag(1:1))//'_tau'//TRIM(char_tau(1:4))//TRIM(tag)
  ELSE
     outfile = 'model_M'//TRIM(char_mpix(1:3))//'_SFH'//&
          TRIM(char_flag(1:1))//'_t'//TRIM(char_tau(1:4))//TRIM(tag)
  ENDIF
