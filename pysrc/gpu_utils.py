...
def _draw_image_cudac(expected_nums, fluxes, N_scale, tolerance=0.01, d_block=32):
    ...
    poiss_sum(generator.state, cuda.In(expected_nums), cuda.In(fluxes), N_bands, N_bins, N_scale,
              cuda.Out(result), block=block_dim, grid=grid_dim)

    #Add on flux from fully-populated bins
    result = np.array([result[i] + fixed_fluxes[i] for i in range(N_bins)]).astype(float)
    return result

def _draw_image_pycuda(expected_nums, fluxes, N_scale, tolerance=0.01, **kwargs):
    assert(_GPU_AVAIL)
    ...
    generator = pycuda.curand.XORWOWRandomNumberGenerator()
    ...
    for b in np.arange(N_bins):
        ...
        if (n_expected <= upper_lim):
            n_stars = rng.gen_poisson(N_scale*N_scale, np.uint32, n_expected).get()
...
def _draw_image_numpy(expected_nums, fluxes, N_scale, fixed_seed=False, tolerance=0.01, **kwargs):
    N_bins = len(expected_nums)
    assert(N_bins == fluxes.shape[1])
    upper_lim = tolerance**-2.
    if fixed_seed:
